# Code adapted from https://github.com/jpcsmith/wf-tools/tree/master
...
class ModifiedKerasClassifier(KerasClassifier):
...
class VarCNNKerasClassifier(ModifiedKerasClassifier):
...
def dilated_basic_1d(
    ...
    def dilated_basic_1d_block(x):
        ...
        y = layers.BatchNormalization(
            epsilon=1e-5,
            name=f"bn{stage_char}{block_char}_branch2a_{suffix}",
        )(y)
        y = layers.Activation(
            "relu",
            name=f"res{stage_char}{block_char}_branch2a_relu_{suffix}",
        )(y)
        ...
        y = layers.BatchNormalization(
            epsilon=1e-5,
            name=f"bn{stage_char}{block_char}_branch2b_{suffix}",
        )(y)
        ...
        if block == 0:
            ...
            shortcut = layers.BatchNormalization(
                epsilon=1e-5,
                name=f"bn{stage_char}{block_char}_branch1_{suffix}",
            )(shortcut)
        else:
            shortcut = x

        y = layers.Add(name=f"res{stage_char}{block_char}_{suffix}")([y, shortcut])
        y = layers.Activation(
            "relu", name=f"res{stage_char}{block_char}_relu_{suffix}"
        )(y)
        return y
