...
class MutualInformationAnalyzer(object):
    ...
    def _estimate_entropy(self, features, site):
        ...
        for feature in features:
            ...
            bw_f = (
                0.9 * min(np.std(X_f), stats.iqr(X_f) / 1.34) * X_f.shape[0] ** (-0.2)
            )
    ...
    def _estimate_nmi(self, feature_pair):
        ...
        mi_1 = self._mi_cache.get(f"{c},{c}", None)
        mi_2 = self._mi_cache.get(f"{r},{r}", None)
        if mi_1 is None:
            mi_1 = self._avg_mi((c, c))
            self._mi_cache[f"{c},{c}"] = mi_1
        if mi_2 is None:
            mi_2 = self._avg_mi((r, r))
            self._mi_cache[f"{r},{r}"] = mi_2
    ...
    def _check_redundancy(self, feature_pair):
        ...
        log.debug(f"| nmi({feature1},{feature2}) = {nmi}")

        # prune if nmi is above threshold
        if nmi > self.nmi_threshold:
            log.debug(f"Feature #{feature1} is redundant w/ #{feature2}")
    ...
    def prune(self, features, checkpoint=None, nmi_threshold=0.9, topn=100):
        ...
        if checkpoint is not None:
            if os.path.exists(checkpoint):
                ...
                features = list(
                    filter(
                        lambda f: f not in cleaned_features
                        and f not in pruned_features,
                        features,
                    )
                )
                checkpoint_fi.close()
        ...
        while features and len(cleaned_features) < topn:
            ...
            for res in results:
                ...
                if checkpoint is not None:
                    checkpoint.write(f"={feature_pair[0]},{feature_pair[1]},{nmi}\n")
                    checkpoint.flush()
            ...
            if not is_redundant:
                ...
                log.info(
                    f"Progress MI: {len(cleaned_features)}/{min(topn, len(features))}"
                )
                if checkpoint is not None:
                    checkpoint.write(f"+{current_feature}\n")
                    checkpoint.flush()
            else:
    ...
    def cluster(self, features, checkpoint=None, min_samples=1, min_cluster_size=3):
        ...
        X = np.zeros(
            shape=(len(features), len(features)), dtype=float
        )  # distance matrix
        pairs = list(
            combinations_with_replacement(features, 2)
        ...
        if len(self._nmi_cache) > 0:
            # ignore unselected features in cache
            cache = [
                (pair, nmi)
                for pair, nmi in self._nmi_cache
                if pair[0] in features and pair[1] in features
            ]
            # add each cached nmi to the distance matrix
            for cached_pair, nmi in cache:
                assert not np.isnan(nmi), cached_pair
                # remove cached_pair from pairs
                pairs = list(
                    filter(
                        lambda pair: (
                            pair[0] != cached_pair[0] and pair[1] != cached_pair[1]
                        )
                        and (pair[0] != cached_pair[1] and pair[1] != cached_pair[0]),
                        pairs,
                    )
                )
        ...
        if len(pairs) > 0:
            ...
            for pair, nmi in zip(pairs, results):
                assert not np.isnan(nmi), pair
                # print progress updates
                count += 1
                if count - 1 % (len(pairs) * 0.05) == 0:
                    log.info(f"Progress MI: {count}/{len(pairs)}")
                ...
                if checkpoint is not None:
                    chk_fi.write(f"={fidx1},{fidx2},{nmi}\n")
                    chk_fi.flush()
        ...
        log.debug(f"Found {set(labels)} clusters.")
