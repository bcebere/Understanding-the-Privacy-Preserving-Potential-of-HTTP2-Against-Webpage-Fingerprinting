...
def prepare_requests(requests):
    out = []
    server_data = json.load(open("data/server_db.json"))  # simulation of a HEAD request
    for request in requests:
        expected_size = None
        if (
            request["path"] in server_data
            and "data_size" in server_data[request["path"]]
        ):
            expected_size = server_data[request["path"]]["data_size"]
