...
case "$DEF" in
  alpaca)  exec python "$HERE"/server_with_defs.py --dst_port "$PORT" --defense_alpaca 1  --level "$LVL" ;;
  tamaraw) exec python "$HERE"/server_with_defs.py --dst_port "$PORT" --defense_tamaraw 1 --level "$LVL" ;;
  h2ps)    HPACK=$(python3 -c "from server_defenses.levels import params; print(params('h2ps','$LVL')['hpack'])")
           exec python "$HERE"/server_simple.py --dst_port "$PORT" --level "$LVL" \
  ...
  nop)     exec python "$HERE"/server_simple.py --dst_port "$PORT" ;;
