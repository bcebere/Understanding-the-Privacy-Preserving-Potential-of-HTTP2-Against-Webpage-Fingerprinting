...
parser.add_argument(
    "-batch", "--batch", dest="batch", help="Use hints and batch requests", default=1
)
parser.add_argument(
...
class AsynEventWithTimeout(asyncio.Event):
...
class H2Protocol(asyncio.Protocol):
    def __init__(self, requests: list) -> None:
        ...
        self.exit_event = AsynEventWithTimeout()
        self.inject_pool = []
    ...
    def send_request(self, path, body, headers):
        ...
        for key in headers:
            request_headers.append((key.lower(), headers[key]))
        next_stream = self.conn.get_next_available_stream_id()
        print("request", path, next_stream, headers)
    ...
    def handle_next_request(self, custom_path=None):
        ...
        if "delay" in next_request:
            delay = next_request["delay"]
            time.sleep(delay)
        headers = {}
        if "headers" in next_request:
            headers = next_request["headers"]
        self.send_request(path, body, headers)

    def _inject_noise(self):
        use_noise = random.choice([True, False])
        if not use_noise:
            return
        if len(self.inject_pool) == 0:
            return

        padding_path = random.choice(self.inject_pool)
        self.handle_next_request(custom_path=padding_path)

        print("padding path", padding_path)
        ...
        try:
            ...
            for event in events:
                if isinstance(event, SettingsAcknowledged):
                    ...
                    def _parse_hints(hint):
                        return hint.split(";")[0].split("<")[-1].split(">")[0]

                    take_hints = random.randrange(min(len(event.headers), 10))
                    for key, hint in event.headers:
                        if key != "link":
                            continue
                        try:
                            hinted_path = _parse_hints(hint)
                            if hinted_path.startswith("inject"):
                                self.inject_pool.append(hinted_path)
                        except BaseException as e:
                            print("Early Hints failure", e)
                            continue

                    print(self.inject_pool)
    ...
    async def wait_for_exit(self):
        await self.exit_event.wait(timeout=5)
